-- This is needed for the decompiler to work in some mobile exploits (e.g. Delta)
local b = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"
local function enc(data)
	return ((data:gsub('.', function(x) 
		local r,b='',x:byte()
		for i=8,1,-1 do r=r..(b%2^i-b%2^(i-1)>0 and '1' or '0') end
		return r;
	end)..'0000'):gsub('%d%d%d?%d?%d?%d?', function(x)
		if (#x < 6) then return '' end
		local c=0
		for i=1,6 do c=c+(x:sub(i,i)=='1' and 2^(6-i) or 0) end
		return b:sub(c+1,c+1)
	end)..({ '', '==', '=' })[#data%3+1])
end

assert(request, "Your exploit does not support request")
assert(getscriptbytecode, "Your exploit does not support getscriptbytecode")

local HttpService = game:GetService("HttpService")
local getscriptbytecode = getscriptbytecode
local request = request

local function decompile(s)
	local response = request({
		Url = "https://starhub.dev/api/v1/decompile",
		Method = "POST",
		Headers = {
			["Content-Type"] = "application/json",
			["Authorization"] = "SignalHub",
		},
		Body = HttpService:JSONEncode({
			bytecode = enc(getscriptbytecode(s)),
		}),
	})

	return response.StatusCode == 200 and response.Body or "Failed to decompile"
end

getgenv().decompile = decompile
